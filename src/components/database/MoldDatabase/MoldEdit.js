...
const MoldEdit = () => {
  ...
  const [OriginalMoldNo, setOriginalMoldNo] = useState("");
  ...
  useEffect(() => {
    if (RowId) {
      ...
      if (mold) {
        ...
        setOriginalMoldNo(mold.Mold_No);
  ...
  const SaveData = () => {
    ...
    // Prepare parts array for saving
    ...
    const updatedData = StoredMoldData.map((m) =>
      m.id === SelectedRowId ? finalMold : m
    );
    sessionStorage.setItem("MoldData", JSON.stringify(updatedData));
  ...
  const SubmitData = () => {
    ...
    if (moldData.Mold_No !== OriginalMoldNo) {
      const StoredMoldData = JSON.parse(sessionStorage.getItem("MoldData")) || [];
      const exists = StoredMoldData.some(
        (m) => m.id !== SelectedRowId && String(m.Mold_No).toLowerCase() === String(moldData.Mold_No).toLowerCase()
      );
      if (exists) {
    ...
    <>
      ...
      <div className="container-fluid">
        <div className="d-flex justify-content-between ml-3 pt-3 pb-3">
          <BreadCrumb DB_Name={"Mold"} Current_Page={"Edit"} />
        </div>
